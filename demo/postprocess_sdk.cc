...
{
  /* model_zoo 模型的输出是 sigmoid 之后的概率（model_zoo 的 post_process 不套 sigmoid 且工作正常）。
     旧逻辑对输出再套一次 sigmoid → 双重 sigmoid 失真 → 64 垃圾框。改成 model_zoo 的 i8 解码：
     阈值直接用 threshold，置信度 = deqnt(class) * deqnt(box_conf)。 */
  int    validCount = 0;
    ...
    for (int i = 0; i < grid_h; i++) {
      for (int j = 0; j < grid_w; j++) {
        ...
        if (box_confidence >= thres_i8) {
          ...
          float limit_score = deqnt_affine_to_f32(maxClassProbs, zp, scale) *
                 ...
                 std::vector<float>& qnt_scales, detect_result_group_t* group)
{
  ...
  if (init == -1) {
    ...
    if (ret < 0) {
      printf("warn: no labels, continuing\n");
    }
  ...
  for (int i = 0; i < validCount; ++i) {
    ...
    group->results[last_count].prop       = obj_conf;
        char* label = (labels && labels[id]) ? labels[id] : (char*)"";
