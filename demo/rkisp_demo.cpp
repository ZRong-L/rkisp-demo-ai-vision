...
static perf_counter_t g_perf_draw   = { .name = "AI 画框回写" };
static perf_counter_t g_perf_vop    = { .name = "AI VOP 显示" };
static perf_counter_t g_perf_ai_total = { .name = "AI 流总耗时" };
#endif
static bool _if_quit = false;
static char g_model_path[256] = "./model/yolov5s-640-640.rknn"; /* 默认相对路径，可用 --model 覆盖 */
...
#include <linux/dma-buf.h>   // 定义 DMA_BUF_IOCTL_SYNC、struct dma_buf_sync
...
static int read_frame(demo_context_t *ctx)
{
    ...
    if (ctx->vop) {
        ...
        if (strlen(ctx->dev_name) && strlen(ctx->dev_name2) && ctx->dev_using != 1) {
        //双路同显，不需要
        // if (ctx->dev_using != 1 && ctx->dev_using != 2) {
...
#else
                printf("SP frame: fd=%d seq=%lu\n", ctx->buffers[i].export_fd, buf.sequence);
#endif
            }
        } else {
#else
...
#endif
            static int dbg_d = 0;
            if (rkaiq_infer_is_ready()) {
                rkaiq_detect_result_group_t yolo_res = {0};
                int n = rkaiq_infer_process(
                ...
                if (n > 0) {
// #ifdef DEBUG_USE_TIME
//                     uint64_t t_draw_start = get_timestamp();   /* 画框回写 开始 */
// #endif
                    // 1. 【开始前】告诉内核，CPU 即将写入
                    struct dma_buf_sync sync_start = {
                        .flags = DMA_BUF_SYNC_START | DMA_BUF_SYNC_WRITE
                    };
                    if (ioctl(ctx->buffers[i].export_fd, DMA_BUF_IOCTL_SYNC, &sync_start)) {
                        fprintf(stderr, "DMA_BUF_SYNC_START failed: fd=%d, flags=0x%x, errno=%d (%s)\n",
                                ctx->buffers[i].export_fd, sync_start.flags, errno, strerror(errno));
                    }
// #ifdef DEBUG_USE_TIME
//                     uint64_t t_sync1 = get_timestamp();
// #endif
                    rkaiq_draw_boxes_nv12(
                        (uint8_t*)ctx->buffers[i].start,
                        ctx->width, ctx->width, ctx->height,
                        &yolo_res);
// #ifdef DEBUG_USE_TIME
//                     uint64_t t_draw_mid = get_timestamp();
// #endif
                    // 3. 【结束后】关键步骤：将 CPU Cache 刷回物理内存（Flush）
                    struct dma_buf_sync sync_end = {
                        .flags = DMA_BUF_SYNC_END | DMA_BUF_SYNC_WRITE
                    };
                    if (ioctl(ctx->buffers[i].export_fd, DMA_BUF_IOCTL_SYNC, &sync_end)) {
                        fprintf(stderr, "DMA_BUF_SYNC_END failed: fd=%d, flags=0x%x, errno=%d (%s)\n",
                                ctx->buffers[i].export_fd, sync_end.flags, errno, strerror(errno));
                    }
// #ifdef DEBUG_USE_TIME
//                     uint64_t t_draw_end = get_timestamp();   /* 画框回写 结束 */
//                     perf_sample(&g_perf_draw, (double)(t_draw_end - t_draw_start));
//                     if ((dbg_d % 2) == 0)
//                         printf("[T] draw sync1=%ldus draw=%ldus sync2=%ldus\n",
//                                (long)(t_sync1 - t_draw_start),
//                                (long)(t_draw_mid - t_sync1),
//                                (long)(t_draw_end - t_draw_mid));
// #endif
                }
// #ifdef DEBUG_USE_TIME
//                 uint64_t t_enc0 = get_timestamp();
// #endif
                rkaiq_encoder_submit(ctx->buffers[i].export_fd);
// #ifdef DEBUG_USE_TIME
//                 uint64_t t_enc1 = get_timestamp();
//                 if ((dbg_d % 2) == 0)
//                     printf("[T] enc=%ldus\n", (long)(t_enc1 - t_enc0));
// #endif
            }
// #ifdef DEBUG_USE_TIME
//             uint64_t t_vop_start = get_timestamp();   /* VOP 显示 开始（drmDspFrame 前） */
// #endif
            drmDspFrame(ctx->width, ctx->height, dispWidth, dispHeight, ctx->buffers[i].export_fd,
                       ctx->buffers[i].start, DRM_FORMAT_NV12);
#ifdef DEBUG_USE_TIME
            uint64_t _t4 = get_timestamp();   /* T4: Smart0 VOP 最终显示 结束 */
            // g_perf_ai.ai_t4 = _t4;
            // perf_sample(&g_perf_vop, (double)(_t4 - t_vop_start));  /* VOP 显示 耗时 */
            if (g_perf_ai.ai_t1)
                perf_sample(&g_perf_ai_total, (double)(_t4 - g_perf_ai.ai_t1));  /* AI 流总耗时+帧率 */
            // if ((dbg_d++ % 2) == 0)
            //     printf("[T] vop=%ldus frame_total=%ldus\n",
            //            (long)(_t4 - t_vop_start), (long)(_t4 - g_perf_ai.ai_t1));
#endif
        }
    }
#endif
...
#endif
    if (ctx->dev_using == 1) rkaiq_encoder_wait_rga();
...
static void parse_args(int argc, char **argv, demo_context_t *ctx)
{
    ...
    while (1) {
        ...
        c = getopt_long(argc, argv, "w:h:f:i:g:j:d:o:c:n:k:a:t:1:2:3Mmpsevrl",
        ...
        case 'p':
            ERR("Usage: %s to capture rkisp1 frames\n"
                ...
                "         --model <path>,                    optional, rknn model file path, default ./model/yolov5s-640-640.rknn\n"
...
#ifndef ANDROID
            //使用默认的效果文件
            printf("use default iq files in /oem/etc/iqfiles\n");
...
#if ISPDEMO_ENABLE_RGA
        if (0) { /* disable display_init, use initDrmDsp instead */
...
#if ISPDEMO_ENABLE_DRM
    printf("ISPDEMO_ENABLE_DRM enabled\n");
#endif
#ifdef ISPFEC_API
    printf("ISPFEC_API enabled\n");
#endif
#if ISPDEMO_ENABLE_RGA
    printf("ISPDEMO_ENABLE_RGA enabled\n");
#endif
    //分配模型输入 buffer 
    if (preprocess_init() < 0)
      printf("preprocess_init failed\n");
    if (rkaiq_infer_init(g_model_path) < 0)
      printf("rkaiq_infer_init failed\n");
    if (rkaiq_encoder_init(main_ctx.width, main_ctx.height) < 0)
      printf("rkaiq_encoder_init failed\n");
